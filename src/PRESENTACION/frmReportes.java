...
public class frmReportes extends JFrame {
    ...
    private void generarReporte() {
        ...
        // ✅ VALIDACIÓN: Verificar que se haya seleccionado un reporte
        ...
        try {
            ...
            switch (tipoReporte) {
                ...
                case "Ventas por Período":
                    sql = "SELECT DATE(p.fecha) AS fecha, COUNT(*) AS total_pedidos, SUM(p.total) AS monto_total " +
                          "FROM pedido p " +
                ...
                case "Productos Más Vendidos":
                    sql = "SELECT pl.nombre, COUNT(dp.id_detalle) AS veces_vendido, " +
                          "SUM(dp.cantidad) AS cantidad_total, SUM(dp.subtotal) AS ingresos " +
                ...
                case "Clientes Frecuentes":
                    sql = "SELECT CONCAT(per.nombre, ' ', per.apellidos) AS cliente, per.ci, " +
                          "COUNT(p.id_pedido) AS total_pedidos, SUM(p.total) AS total_gastado " +
                          "FROM cliente c " +
                          "INNER JOIN persona per ON c.id_persona = per.id_persona " +
                ...
                case "Reservas del Mes":
                    sql = "SELECT r.id_reserva, CONCAT(per.nombre, ' ', per.apellidos) AS cliente, " +
                          "m.numero_mesa, r.fecha, r.hora, r.personas, r.estado " +
                          "FROM reserva r " +
                ...
                case "Inventario de Platos":
                    sql = "SELECT c.nombre AS categoria, p.nombre AS plato, p.precio, " +
                          "p.disponible, p.tiempo_preparacion_min " +
                          "FROM plato p " +
                ...
                case "Ventas con Descuentos":
                    sql = "SELECT f.numero_factura, f.fecha_hora, " +
                          ...
                          "INNER JOIN cliente c ON p.id_cliente = c.id_cliente " +
                          "INNER JOIN persona per ON c.id_persona = per.id_persona " +
                          "WHERE f.descuento > 0 AND DATE(f.fecha_hora) BETWEEN '" + fechaInicio + "' AND '" + fechaFin + "' " +
                ...
                case "Clientes VIP (Fidelidad)":
                    sql = "SELECT CONCAT(per.nombre, ' ', per.apellidos) AS cliente, per.ci, " +
                          "c.puntos_fidelidad, COUNT(p.id_pedido) AS total_pedidos, " +
