...
public class frmMenuAdministracion extends JFrame {
    ...
    private void inicializarComponentes() {
        ...
        panelContenido.setPreferredSize(new Dimension(920, 1400));
        ...
        crearSeccion(panelContenido, "GESTIÓN DE INVENTARIO", 20, 400, new String[][]{
            {"Control de Ingredientes", "Gestionar stock, ingredientes y alertas"},
        ...
        // CONFIGURACIÓN
        crearSeccion(panelContenido, "CONFIGURACIÓN", 20, 780, new String[][]{
            {"Parámetros del Sistema", "Configurar datos generales y de negocio"},
            {"Respaldar Base de Datos", "Crear, descargar o restaurar copias de seguridad"}
        }, new Runnable[]{
            () -> mostrarParametrosSistema(),
    ...
    private void crearSeccion(JPanel panelPadre, String titulo, int x, int y, String[][] botones, Runnable[] acciones) {
        JLabel lblSeccion = new JLabel("GESTIÓN DE PRODUCTOS");
    ...
    // PARÁMETROS DEL SISTEMA (desarrollado)
    private void mostrarParametrosSistema() {
        JDialog dialog = new JDialog(this, "Parámetros del Sistema", true);
        dialog.setSize(520, 380);
        dialog.getContentPane().setLayout(null);
        dialog.setLocationRelativeTo(this);

        JLabel lblTitulo = new JLabel("Modificar parámetros generales del sistema:");
        lblTitulo.setFont(new Font("Segoe UI", Font.BOLD, 16));
        lblTitulo.setBounds(40, 30, 420, 25);
        dialog.getContentPane().add(lblTitulo);

        JLabel lblRazon = new JLabel("Razón Social / Nombre:");
        lblRazon.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        lblRazon.setBounds(40, 80, 170, 22);
        dialog.getContentPane().add(lblRazon);

        JTextField tbxRazon = new JTextField();
        tbxRazon.setBounds(220, 80, 230, 26);
        dialog.getContentPane().add(tbxRazon);

        JLabel lblDireccion = new JLabel("Dirección:");
        lblDireccion.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        lblDireccion.setBounds(40, 120, 170, 22);
        dialog.getContentPane().add(lblDireccion);

        JTextField tbxDireccion = new JTextField();
        tbxDireccion.setBounds(220, 120, 230, 26);
        dialog.getContentPane().add(tbxDireccion);

        JLabel lblTelefono = new JLabel("Teléfono:");
        lblTelefono.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        lblTelefono.setBounds(40, 160, 170, 22);
        dialog.getContentPane().add(lblTelefono);

        JTextField tbxTelefono = new JTextField();
        tbxTelefono.setBounds(220, 160, 230, 26);
        dialog.getContentPane().add(tbxTelefono);

        JLabel lblImpuesto = new JLabel("Impuesto (%):");
        lblImpuesto.setFont(new Font("Segoe UI", Font.PLAIN, 14));
        lblImpuesto.setBounds(40, 200, 170, 22);
        dialog.getContentPane().add(lblImpuesto);

        JTextField tbxImpuesto = new JTextField();
        tbxImpuesto.setBounds(220, 200, 230, 26);
        dialog.getContentPane().add(tbxImpuesto);

        // Cargar los parámetros desde un archivo Java properties (parametros.properties)
        Properties prefs = new Properties();
        try {
            File f = new File("parametros.properties");
            if (f.exists()) {
                prefs.load(new FileInputStream(f));
                tbxRazon.setText(prefs.getProperty("razon",""));
                tbxDireccion.setText(prefs.getProperty("direccion",""));
                tbxTelefono.setText(prefs.getProperty("telefono",""));
                tbxImpuesto.setText(prefs.getProperty("impuesto","13"));
            }
        } catch(Exception ex) {}

        JButton btnGuardar = new JButton("Guardar");
        UIConstants.estilizarBoton(btnGuardar, UIConstants.COLOR_SUCCESS);
        btnGuardar.setBounds(120, 270, 120, 35);
        btnGuardar.addActionListener(e -> {
            try {
                prefs.setProperty("razon", tbxRazon.getText());
                prefs.setProperty("direccion", tbxDireccion.getText());
                prefs.setProperty("telefono", tbxTelefono.getText());
                prefs.setProperty("impuesto", tbxImpuesto.getText());
                prefs.store(new FileOutputStream("parametros.properties"), "Parámetros Cafetería");
                JOptionPane.showMessageDialog(dialog,
                    "¡Parámetros guardados con éxito!", "Parámetros", JOptionPane.INFORMATION_MESSAGE);
                dialog.dispose();
            } catch(Exception ex) {
        ...
        btnCerrar.setBounds(270, 270, 120, 35);
    ...
    // RESPALDAR BASE DE DATOS (desarrollado, MySQL)
    private void respaldarBaseDeDatos() {
        ...
        if(userSelection == JFileChooser.APPROVE_OPTION){
            ...
            try (Connection conn = conexion.conectar()) {
                ...
                // Cambia estos según tu configuración
                String mysqldump = "mysqldump"; // Asegúrate que esté en PATH/env
                String user = "root";
                String password = "root";

                String backupPath = carpeta.getAbsolutePath() + File.separator + db +
                        "_backup_" + System.currentTimeMillis()/1000 + ".sql";

                String cmd = String.format("%s -u%s -p%s --add-drop-table --routines --events %s -r \"%s\"",
                        mysqldump, user, password, db, backupPath);

                Process runtimeProcess = Runtime.getRuntime().exec(cmd);
                int processComplete = runtimeProcess.waitFor();
                if(processComplete==0){
                    JOptionPane.showMessageDialog(this, 
                        "Respaldo generado: " + backupPath, 
                        "Backup Exitoso", JOptionPane.INFORMATION_MESSAGE);
                }else{
                    JOptionPane.showMessageDialog(this, 
                        "No se pudo generar el respaldo.\nVerifique contraseña o permisos", 
