...
public class frmDashboard extends JFrame {
    ...
    private void inicializarComponentes() {
        ...
        getContentPane().add(panelTopPlatos);
        ...
        getContentPane().add(panelTopClientes);
    ...
    private void cargarDatos() {
        ...
        try (Connection conn = conexion.conectar()) {
            ...
            String sqlVentasHoy = "SELECT COALESCE(SUM(total), 0) AS total FROM pedido WHERE DATE(fecha) = CURDATE() AND estado = 'Pagado'";
            ...
            String sqlPedidosHoy = "SELECT COUNT(*) AS total FROM pedido WHERE DATE(fecha) = CURDATE()";
            ...
            String sqlTopPlatos = "SELECT * FROM v_productos_mas_vendidos ORDER BY cantidad_total DESC LIMIT 5";

            try (PreparedStatement ps = conn.prepareStatement(sqlTopPlatos);
                 ResultSet rs = ps.executeQuery()) {
                int pos = 1;
                while (rs.next()) {
                    sbPlatos.append(String.format("%d. %s (%s)\n",
                            pos++,
                            rs.getString("nombre"),
                            rs.getString("categoria")
                    ));

                    sbPlatos.append(String.format(
                            "   Veces Pedido: %d | Cantidad: %d | Ingresos: %s\n\n",
                            rs.getInt("veces_pedido"),
                            rs.getInt("cantidad_total"),
                            formatoMoneda.format(rs.getDouble("ingresos_totales"))
                    ));
                }
            }
            ...
            String sqlTopClientes = "SELECT * FROM v_clientes_vip ORDER BY puntos_fidelidad DESC LIMIT 5";
            ...
            try (PreparedStatement ps = conn.prepareStatement(sqlTopClientes);
                ...
                while (rs.next()) {
                    ...
                    sbClientes.append(String.format(
                            "   Nivel: %s | Puntos: %d | Visitas: %d | Gastado: %s\n\n",
                            rs.getString("nivel_fidelidad"),
                            rs.getInt("puntos_fidelidad"),
                            rs.getInt("cantidad_visitas"),
                            formatoMoneda.format(rs.getDouble("total_gastado"))
                    ));
                }
            }
